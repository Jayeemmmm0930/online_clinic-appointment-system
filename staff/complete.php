...
$sql = mysqli_query($connection, "SELECT * FROM tbl_transaction WHERE status = 'Complete'");
...
if (mysqli_num_rows($sql) > 0) {
    while ($rows = mysqli_fetch_array($sql)) {
        ...
        $statusStyle = ($rows['status'] == 'Complete') ? 'background-color: green; color: white; border-radius: 5px; padding: 2px 5px;' : '';
