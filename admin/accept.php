...
$sql = mysqli_query($connection, "SELECT * FROM tbl_transaction WHERE status = 'Approve'");
...
if (mysqli_num_rows($sql) > 0) {
    while ($rows = mysqli_fetch_array($sql)) {
        ...
        $statusStyle = ($rows['status'] == 'Approve') ? 'background-color: lightgreen; color: white; border-radius: 5px; padding: 2px 5px;' : '';
        ...
        echo "<td>";
        if ($rows['status'] == 'Approve') {
            echo "<a class='approvebtn' data-id='" . $rows["id"] . "' href='#' onclick='confirmAction(" . $rows["id"] . ", \"complete\")'><i class='bx bxs-calendar-check' style='color:lightblue; font-size: 40px;' ><span class='tooltip'>Complete</span></i></a>";
            echo "<a class='rejectbtn' data-id='" . $rows["id"] . "' href='#' onclick='confirmAction(" . $rows["id"] . ", \"unsuccessful\")'><i class='bx bxs-user-x' style='color:lightcoral; font-size: 40px;' ><span class='tooltip'>Unsuccessful</span></i></a>";
...
function confirmAction(id, action) {
    var message = (action === "complete") ? "Are you sure you want to mark this as complete?" : "Are you sure you want to mark this as unsuccessful?";
    var result = confirm(message);
    if (result) {
        if (action === "complete") {
            completeAction(id);
        } else if (action === "unsuccessful") {
            unsuccessfulAction(id);
        }
    } else {
        console.log(action.charAt(0).toUpperCase() + action.slice(1) + " action canceled by user.");
    }
}
...
function completeAction(id) {
    $.ajax({
        type: "POST",
        url: "../querys/complete_status.php",
        data: { id: id, status: "complete" },
        success: function(response) {
            console.log("Response from server:", response);
            console.log("Complete action successful for ID: " + id);
            location.reload(); 
        },
...
function unsuccessfulAction(id) {
    $.ajax({
        type: "POST",
        url: "../querys/unsuccessful.php",
        data: { id: id, status: "unsuccessful" },
        success: function(response) {
            console.log("Response from server:", response);
            console.log("Unsuccessful action successful for ID: " + id);
            location.reload(); 
        },
        error: function(xhr, status, error) {
            console.error("Error updating status:", error);
