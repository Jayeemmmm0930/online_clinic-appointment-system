...
$sql = mysqli_query($connection, "SELECT * FROM tbl_transaction WHERE status = 'Pending'");
...
if (mysqli_num_rows($sql) > 0) {
    while ($rows = mysqli_fetch_array($sql)) {
        ...
        $statusStyle = ($rows['status'] == 'Pending') ? 'background-color: lightblue; color: white; border-radius: 5px; padding: 2px 5px;' : '';
        ...
        if ($rows['status'] == 'Pending') {
            echo "<a class='approvebtn' data-id='" . $rows["id"] . "' href='#' onclick='confirmApproval(" . $rows["id"] . ")'><i class='bx bxs-check-circle' style='color:lightgreen; font-size: 40px;'><span class='tooltip'>Approve</span></i></a>";
            echo "<a class='rejectbtn' data-id='" . $rows["id"] . "' href='#' onclick='confirmRejection(" . $rows["id"] . ")'><i class='bx bxs-x-circle' style='color:lightcoral; font-size: 40px;' ><span class='tooltip'>Reject</span></i></a>";
        }
        echo "</td>";
        echo "</tr>";
